...
class NodeExtractTimeOnly(NodeTemplate):

    ndname = _('Extract Time Only')

    action = 'extract-time-only'
    ...
    def do_process(self,
        ...
        if self.frame.data['columns']:
            ...
            table = table.with_columns(col(column).dt.time())
    ...
    def _refresh_column(self) -> None:
        ...
        table_columns = table.select(cs.time() | cs.datetime()) \
                             .collect_schema() \
                             .names()
