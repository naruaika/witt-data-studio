...
class NodeExtractDateOnly(NodeTemplate):

    ndname = _('Extract Date Only')

    action = 'extract-date-only'
    ...
    def do_process(self,
        ...
        if self.frame.data['columns']:
            ...
            table = table.with_columns(col(column).dt.date())
