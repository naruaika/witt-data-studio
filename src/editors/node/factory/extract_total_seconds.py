...
class NodeExtractTotalSeconds(NodeTemplate):

    ndname = _('Extract Total Seconds')

    action = 'extract-total-seconds'
    ...
    def new(x:   int = 0,
        ...
        """"""
        self = NodeExtractTotalSeconds(x, y)
    ...
    def do_process(self,
        ...
        if self.frame.data['columns']:
            ...
            expr = col(column).dt.total_seconds(fractional = True)
    ...
    def _refresh_column(self) -> None:
        ...
        table_columns = table.select(cs.duration()) \
                             .collect_schema() \
                             .names()
