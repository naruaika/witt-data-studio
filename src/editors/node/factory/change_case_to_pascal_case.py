...
class NodeChangeCaseToPascalCase(NodeTemplate):

    ndname = _('Change Case to Pascal Case')

    action = 'change-case-to-pascal-case'
    ...
    def new(x:   int = 0,
        ...
        """"""
        self = NodeChangeCaseToPascalCase(x, y)
    ...
    def do_process(self,
        ...
        if self.frame.data['columns']:
            ...
            norm_str = col(column).str.replace_all(r'([a-z])([A-Z])', r'${1} ${2}') \
            ...
            expr = words.list.eval(part_1 + part_2).list.join('')
