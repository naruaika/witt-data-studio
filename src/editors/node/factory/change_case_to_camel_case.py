...
class NodeChangeCaseToCamelCase(NodeTemplate):

    ndname = _('Change Case to Camel Case')

    action = 'change-case-to-camel-case'
    ...
    def new(x:   int = 0,
        ...
        """"""
        self = NodeChangeCaseToCamelCase(x, y)
    ...
    def do_process(self,
        ...
        if self.frame.data['columns']:
            ...
            expr = self._build_to_pascal_case_expr(col(column))
            expr = expr.str.slice(0, 1).str.to_lowercase() + \
                   expr.str.slice(1, None)
            table = table.with_columns(expr.alias(column))

        self.frame.data['table'] = table

    def _build_to_pascal_case_expr(self,
                                   expr: Expr,
                                   ) ->  Expr:
        """"""
        from polars import element
        norm_str = expr.str.replace_all(r'([a-z])([A-Z])', r'${1} ${2}') \
                       .str.replace_all(r'[\.\s_-]+', ' ') \
                       .str.strip_chars(' ')
        words = norm_str.str.split(' ')
        part_1 = element().str.slice(0, 1).str.to_uppercase()
        part_2 = element().str.slice(1, None).str.to_lowercase()
        return words.list.eval(part_1 + part_2).list.join('')
