...
@Gtk.Template(resource_path = '/com/wittara/studio/editors/node/editor.ui')
...
#                                               shortcuts  = ['h'])
        ...
        create_action('change-case-to-'
                      'lowercase',              lambda *_: create_node('change-case-to-lowercase'))
        create_action('change-case-to-'
        ...
        create_action('calculate-sine',         lambda *_: create_node('calculate-sine'))
        create_action('calculate-cosine',       lambda *_: create_node('calculate-cosine'))
        create_action('calculate-tangent',      lambda *_: create_node('calculate-tangent'))
    ...
    def _setup_commands(self) -> None:
        ...
        create_command('change-case-to-'
                       'lowercase',             f"{_('Column')}: {_('Change Case to Lowercase')}")
        create_command('change-case-to-'
    ...
    def _on_scrolled(self,
        ...
        def do_restore() -> None:
            """"""
            self.Canvas.set_can_target(True)
            self._scroll_restore_id = None

        if self._scroll_restore_id:
